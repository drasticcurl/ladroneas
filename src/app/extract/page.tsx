...
interface ExtractionResult { slides: AnalyzedSlide[]; funnel_style_notes: string; total_questions: number; ad_copy_insights: string; landing_url: string; slides_extracted: number; cost?: CostInfo }
    ...
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      ...
      {status === "error" && (
        <Card className="mb-8 border-destructive/50"><CardContent className="p-6">
          <p className="font-medium text-destructive mb-1">Error en la extraccion</p><p className="text-sm text-muted-foreground">{error}</p>
          ...
          <Card className="border-green-500/20 bg-green-500/5"><CardContent className="p-6">
            ...
            {result.cost && (
              <div className="bg-background/50 rounded-lg border border-border p-4 mb-4">
                <div className="flex items-center gap-2 mb-2"><span className="text-sm font-medium">Costo del analisis</span><Badge variant="outline" className="text-xs">{result.cost.model}</Badge></div>
